    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <form onSubmit={handleScrape}>
          ...
          {selectedSourceData && (
            <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm"><strong>Selected:</strong> {selectedSourceData.name}</p>
              <p className="text-sm"><strong>URL:</strong> <a href={selectedSourceData.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{selectedSourceData.url}</a></p>
              <p className="text-sm"><strong>Category:</strong> {selectedSourceData.category}</p>
            </div>
          )}
            ...
            <div key={category}>
              ...
              <ul className="space-y-2">
                {NEWS_SOURCES.filter(s => s.category === category).map((source) => (
                  <li key={source.name} className="flex items-center text-sm">
                    <span className="text-green-500 mr-2">✓</span>
                    {source.name}
                    {source.priority === 1 && <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">High Priority</span>}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="bg-gray-100 rounded-lg p-6">
        ...
        <p className="text-sm text-gray-600">
          <strong>Phase 1 Status:</strong> Basic scraping infrastructure complete.
